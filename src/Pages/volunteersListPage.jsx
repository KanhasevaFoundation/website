...
const VolunteersListPage = () => {
  ...
  const getSortIcon = (columnName) => {
    ...
    return sortConfig.direction === "ascending" ? (
      <FaSortUp className="text-sky-600" />
    ) : (
      <FaSortDown className="text-sky-600" />
    );
  };
    ...
    <div className="max-w-6xl mx-auto mt-8 p-4">
      <div className="bg-white rounded-lg shadow-lg overflow-hidden">
        <div className="bg-gradient-to-r from-sky-500 to-sky-600 py-6 px-6">
          ...
          <p className="text-sky-100 mt-1">Displaying {filteredVolunteers.length} registered volunteers</p>
        </div>
        ...
        <div className="p-4 bg-sky-50 border-b">
          <div className="relative">
            <input
              ...
              className="w-full p-3 pl-10 pr-4 rounded-lg border border-sky-200 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent"
            />
            <FaSearch className="absolute left-3 top-1/2 transform -translate-y-1/2 text-sky-400" />
          </div>
        </div>
        ...
        {searchTerm && filteredVolunteers.length === 0 && (
          <div className="text-center py-8">
            <FaExclamationTriangle className="text-sky-300 text-4xl mx-auto mb-2" />
            <p className="text-gray-600">No volunteers found matching "{searchTerm}"</p>
            <button
              onClick={() => setSearchTerm("")}
              className="mt-3 text-sky-600 hover:text-sky-800 underline"
            >
              Clear search
            </button>
          </div>
            ...
            <table className="min-w-full bg-white">
              ...
              <tbody>
                {currentVolunteers.map((volunteer) => (
                  <tr key={volunteer._id} className="border-b border-gray-100 hover:bg-sky-50 transition-colors">
                    <td className="p-4">
                      {volunteer.image ? (
                        <img
                          src={volunteer.image}
