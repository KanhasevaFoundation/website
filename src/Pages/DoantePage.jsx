...
const DonatePage = () => {
  ...
  const handlePayment = async (amount, currency, orderId, keyId, donationId) => {
    const options = {
      key: keyId,
      amount: amount,
      currency: currency,
      name: "Kanha Seva Foundation",
      description: "Donation for Seva",
      order_id: orderId,
      handler: async function (response) {
        try {
          const verifyResponse = await api.post('/api/donate/verify', {
          ...
          if (verifyResponse.data.status === 'Success') {
            ...
            setErrorMessage('');
            // Navigate after short delay or show success modal
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // If money donation, handle Razorpay
      if (donorType === 'Personal' && donationType === 'Money' && response.data.razorpayOrderId) {
        const { amount, currency, razorpayOrderId, keyId, donationId } = response.data;
        handlePayment(amount, currency, razorpayOrderId, keyId, donationId);
      } else {
        // Non-money donation success immediately
      ...
      console.error(error);
      const backendError = error.response?.data?.details || error.response?.data?.error || 'Error submitting donation';
